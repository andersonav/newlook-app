...
const create = async (obj:any) => {
  ...
  if(obj.ID == 0){
    ...
    sql = `
      INSERT INTO ${table} (DESCRICAO, RAZAO_SOCIAL, NOME_FANTAZIA, ENDERECO, NUMERO, CEP, CPF_CNPJ, STATUS, TIPO, ID) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
    `;
  }else{

    sql = `
      UPDATE ${table} SET DESCRICAO = ?, RAZAO_SOCIAL = ?, NOME_FANTAZIA = ?, ENDERECO = ?, NUMERO = ?, CEP = ?, CPF_CNPJ = ?, STATUS = ?, TIPO = ? WHERE ID = ?;
    `;
  }
...
const length = (seach:string, tipo:number) => {
  ...
  if(typeof seach != 'undefined' && seach.length > 0){
    ...
    filtroText += `(LOWER(RAZAO_SOCIAL) LIKE LOWER('%${searchP}%') `+
                `OR LOWER(NOME_FANTAZIA) LIKE LOWER('%${searchP}%') `+
                `OR LOWER(CPF_CNPJ) LIKE LOWER('%${searchP}%'))`;
  }
